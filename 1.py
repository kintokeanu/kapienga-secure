# opencv for all image detection tasks
import cv2, time
#images
from datetime import datetime, time
#combine all images and make a video
import argparse
# file handling
import os

# xml file that has all properties of human face
face_cascade = cv2.CascadeClassifier("haarcascade_frontalface_default.xml")


video = cv2.VideoCapture(0)

#loop to extract data from video variable

while True:
    check, frame = video.read()
    if frame is not None:
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        # detect features of face cascade
        faces = face_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=10)
        for x, y, w, h in faces:
            #draw a rectangular frame around the face
            img = cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 3)
            #record exact time human face was infront of the camera
